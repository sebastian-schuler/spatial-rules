...
- **Query** — one batch evaluation of candidates against a ruleset: a spatial predicate (or `withinDistance` radius), an optional property `where`, optional excluded rule IDs, an opt-in overlap computation (`includeOverlap`, ADR-0012), an optional reference time `at` for temporal predicates (ADR-0017), and an optional `aggregate` spec for per-candidate analytics over the applicable set (ADR-0018).
