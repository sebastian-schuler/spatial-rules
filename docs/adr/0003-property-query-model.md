...
Extended by ADR-0011: the where subset adds field-level `$not`, `$nin`, and `$exists` under the same missing-property = non-match semantics; `$regex`/`$size` stay out of scope; no index extension.
