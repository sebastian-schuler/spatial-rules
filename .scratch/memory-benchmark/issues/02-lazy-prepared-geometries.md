...
- **Batch pre-pass rejected by measurement.** A batch-level design that
  ...
  cache.
- **Result ordering kept deterministic.** The relate loop filters each
  candidate's envelope results to the rules it will relate against, prepares
  exactly the missing ones, then relates in envelope order — so
  `Matched.rule_ids` keeps the eager path's deterministic ascending order
  whether or not the memo was already warm. Pinned by
  `rule_ids_stay_in_envelope_order_with_a_partially_warm_memo`.
...
- 2026-08-23 (post-review): the memo was deepened into a single `PreparedMemo`
  seam in `prepared_cache.rs` (bundling ruleset identity, the rule slice, and
  the shared slots) — removing a leaked `rules_slice()` accessor on `Ruleset`
  and the `(slots, rules)` data clump a review flagged — and the relate loop
  was unified to filter the envelope results in place, `ensure` the missing
  slots, then relate in envelope order (one loop, no post-hoc `rule_ids`
  re-sort; ordering is deterministic by construction). Verified: full suite +
  clippy + node smoke green; memory-scale 100k×100 qps 6.9–7.1 M (within the
  6.0–7.6 M run-variance band), serving footprint unchanged.