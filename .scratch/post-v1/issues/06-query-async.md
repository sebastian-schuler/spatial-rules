...
2026-09-17 (HTTP-serving research): the **threading model in the Design section is out of date**. `#[napi] async fn` does not run on libuv's threadpool in napi-rs 3.x — it runs on a NAPI-RS-managed **Tokio** runtime; `AsyncTask<T>` is the libuv-pool abstraction (napi-rs "Async and concurrency" decision table, https://napi.rs/docs/more/async-concurrency). So `UV_THREADPOOL_SIZE` does not size this pool and the "shares libuv's pool with fs/DNS/crypto/zlib" contention note does not apply. napi-rs also documents that a synchronous CPU-bound body inside `async fn` "still occupies a Tokio worker thread", which is what `queryAsync` does; the documented primitive for blocking work is `AsyncTask` or `spawn_blocking`, and the Tokio worker count is only settable via `create_custom_tokio_runtime` (default count not documented → unmeasured). ADR-0009 carries the same correction. See `.scratch/perf-memory/research/01-http-serving-optimizations.md`.
