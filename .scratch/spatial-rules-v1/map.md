...
- [Prepared-geometry options in the chosen stack](issues/03-prepared-geometries.md) — prepare `PreparedGeometry` per worker (thread-affine by design), relate one-sided; no sharing route without an upstream `Sync` change; ladder E/F decides adoption.
